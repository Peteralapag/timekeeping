    ...
    if ($idcode && $from && $to) {
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $date = isset($row['trans_date']) ? date("F d, Y", strtotime($row['trans_date'])) : '—';
                
                // Convert Time In and Time Out to 12-hour format with AM/PM
                ...
                echo "<tr $rowClass><td>$date</td><td>$timeIn</td><td>$timeOut</td></tr>";
            }
